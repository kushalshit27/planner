...
export function Timeline() {
  ...
  if (list.length === 0) {
    return <p class="text-slate-500 text-sm">No tasks yet. Add tasks or seed from IndexedDB.</p>;
  }
          ...
          {list.map((task: Task, idx: number) => {
            ...
            if (isActive) {
              if (drag.value?.mode === 'move') {
                ...
                const clamped = Math.max(delta, minDelta + drag.value.durationDays - 1);
                displayEnd = drag.value.originEnd.add(clamped, 'day');
              }
            }

            const finalDuration = Math.max(1, displayEnd.diff(displayStart, 'day') + 1);
            ...
            return (
              <div
                ...
                onPointerUp={async (e) => {
                  ...
                  if (drag.value.mode === 'resize-end' && deltaDays !== 0) {
                    const minDuration = 1;
                    const clamped = Math.max(deltaDays, minDuration - drag.value.durationDays);
                  ...
                  await moveTaskDates(task.id, newStart.toDate(), newEnd.toDate());
                  drag.value = null;
                }}
                onPointerCancel={() => {
                  drag.value = null;
              ...
              >
                <div class="flex items-center justify-between gap-2">
                  ...
                  <span class="text-[10px] uppercase tracking-wide opacity-90">{task.status}</span>
                </div>
